...
export const getUserRole = async (userId) => {
  try {
    ...
    if (error && error.code === 'PGRST116') {
      ...
      return { data: { role: 'user', is_active: true }, error: null };
    }

    if (error) throw error;

    return { data: data || { role: 'user', is_active: true }, error: null };
  } catch (error) {
    return { data: null, error };
  }
};
...
// Update user role
export const updateUserRole = async (userId, role, accessExpiresAt = null) => {
  try {
    const { data, error } = await supabase
      .from('user_roles')
      .upsert({
